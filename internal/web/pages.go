...
func (s *Server) home(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.fail(w, http.StatusInternalServerError, "could not list projects", err)
		return
	}
...
func (s *Server) createProject(w http.ResponseWriter, r *http.Request) {
	...
	project, err := s.Store.CreateProject(r.Context(), parent, "", r.FormValue("name"))
	if err != nil {
		s.fail(w, http.StatusBadRequest, "could not create the project", err)
		return
	}
...
func (s *Server) projectPage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.fail(w, http.StatusInternalServerError, "could not list parts", err)
		return
	}
...
func (s *Server) partPage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.fail(w, http.StatusInternalServerError, "could not read the part", err)
		return
	}
	...
	s.render(w, "part.html", map[string]any{
		"Part":     part,
		"Path":     path,
		"Fields":   fields,
		"Files":    partFiles,
...
func (s *Server) download(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, store.ErrNotFound) {
		s.fail(w, http.StatusNotFound, "no such file", nil)
		return
	}
	if err != nil {
	...
	if err != nil {
		s.fail(w, http.StatusInternalServerError, "the stored bytes are missing", err)
		return
	}
	defer content.Close()
...
func (s *Server) printersPage(w http.ResponseWriter, r *http.Request) {
	...
	s.render(w, "printers.html", map[string]any{"Jobs": jobs})
}
